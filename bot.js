...
const slashCommands = [
  ...
  // Roblox
  new SlashCommandBuilder().setName('juego').setDescription('Busca un juego de Roblox y ve sus estadísticas').addStringOption(o => o.setName('nombre').setDescription('Nombre del juego').setRequired(true)),
...
client.on('interactionCreate', async (interaction) => {
  ...
  // Verificar cooldown (ephemeral para no contaminar el chat)
...
const server = http.createServer(async (req, res) => {
  if (req.method === 'POST' && req.url === '/kofi') {
    ...
    res.writeHead(200); res.end('🤖 Bot Roblox v10.0 — Activo');
  }
});
